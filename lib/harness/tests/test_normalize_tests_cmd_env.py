...
def test_rewrites_python_dash_m_pytest(monkeypatch, phase):
...
def test_does_not_rewrite_python_dot_exe_token(monkeypatch, phase):
    monkeypatch.setattr(phase.shutil, "which", _which_only_python3)
...
def test_empty_string_passes_through(monkeypatch, phase):
    monkeypatch.setattr(phase.shutil, "which", _which_only_python3)
    assert phase.normalize_tests_command("") == ""


def test_no_rewrite_when_python_present(monkeypatch, phase):
