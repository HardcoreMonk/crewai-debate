...
def test_fallback_push_failure_resets_local_commit(phase_module, tmp_path, monkeypatch):
    ...
    assert "local empty commit reverted" in log_text


def test_fallback_commit_failure_does_not_push(phase_module, tmp_path, monkeypatch):
    """When the empty commit itself fails (e.g. git config user missing),
    no push attempt and no reset — the working copy is unchanged."""
    phase, state_mod = phase_module
    target_repo = tmp_path / "target"
    target_repo.mkdir()
