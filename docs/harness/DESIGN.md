...
- **#13 Empty-commit fresh-SHA doesn't always elicit CodeRabbit re-review (신규, PR #45 dogfood-impl-timeout-override 검증 중 발견)** — ⚠️ **partial — composite path + reopen fallback validated**. B3-1d hybrid auto-bypass는 의도대로 정확히 작동(rate-limit 감지 → manual `@coderabbitai review` post → CodeRabbit decline 응답 감지 → empty commit `0fe623f0` push, log timeline 검증됨). 그러나 push된 fresh SHA에 대해 CodeRabbit이 30+분 동안 review 미생성, 추가 rate-limit comment도 안 옴. 가능 원인: (a) CodeRabbit hourly bucket 완전 소진 시 silent ignore (추가 rate-limit comment도 안 보냄), (b) empty commit을 "no diff" filter로 자동 skip, (c) "already reviewed commits" 처리가 후속 push까지 일정 시간 적용. PR #47 옵션 (a) fix: empty commit → `.harness/auto-bypass-marker.md` 실제-diff marker 파일 commit으로 교체 (실 diff 있으므로 "no diff" filter 가설 (b) 기각).
  ...
  - Fix 후보 (deferred — PR #50으로 (c) 수동 효과는 검증, 자동화는 빈도 더 쌓이면 결정): (a) marker file에 `[force-review]` 같은 키워드 포함하여 CodeRabbit 트리거 강화 (실증 필요), (b) marker push 후 일정 polling 안에 review 안 오면 또 marker 파일 갱신 commit 시도 (loop with cap, 운영 노이즈 vs 효과 trade-off), (c) **최종 fallback으로 PR 닫고 재오픈** — PR #50으로 효과 검증됨, 자동화 시 trigger 조건은 "review-wait가 timeout=failed로 종결" + "round=1" + "auto_bypass_commit_pushed=true" (marker는 이미 푸시됨). 자동화 비용은 close+reopen API 호출 2건 + bump_round + 재실행 — 작음. 가시 부작용은 PR 상태 changelog에 close/reopen 이벤트가 추가되는 것 (외부 watcher 알림 가능). 운영 정책: silent-ignore가 또 한 번 더 발생하면 (n=2로 빈도 확정) 자동화 PR 진행, 그 전까지는 RUNBOOK 수동 절차 유지.
- **#14 First external-repo dogfooding — feature branch 미리 안 만들면 master에 commit 떨어짐 (2026-04-25 발견, gen-14 `claude-project-mgmt#1`)** — ⏳ **open**. crewai self-dogfood에선 운영자가 자연스럽게 `git checkout -b ...`를 plan 전에 했지만, 외부 repo에 처음 적용 시 누락 → harness `cmd_commit`이 현재 HEAD branch (master) 위에 commit을 그대로 떨어뜨림 → `cmd_pr_create`가 `branch in ("main", "master")` 체크에서 fatal-exit. 운영자가 수동 복구: `git branch <feature> HEAD`로 commit 백업 → `git update-ref refs/heads/master refs/remotes/origin/master`로 master 원복 → `git checkout <feature>`로 이동 후 pr-create 재시도. 복구는 commit lossless이지만 운영 부담 ↑. 영향: harness가 외부 repo로 일반화될수록 빈도 ↑. Fix 후보: (a) `cmd_plan` 또는 `cmd_impl` 시작 시 `git rev-parse --abbrev-ref HEAD == main/master`이면 fail-fast `"checkout a feature branch first"` (현재 pr-create의 메시지를 더 일찍 끌어올림 — 가장 작음, 운영자 제어 유지), (b) `cmd_plan`이 자동 `git checkout -b harness/<slug>` (auto-create, 운영자 제어 ↓, 의도치 않은 branch 생성 위험), (c) `--auto-branch` 플래그로 (b)를 opt-in (둘 다 지원, 복잡도 ↑). 1차 결정: (a) — 이미 pr-create에서 검증된 메시지를 plan/impl 진입에서도 출력. (b)/(c)는 외부 repo 빈도 데이터 누적 후 재검토.
