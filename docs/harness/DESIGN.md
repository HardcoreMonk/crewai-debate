...
- **#15 Pre-marker silent-ignore subtype (2026-04-26 발견, PR #60 round 1 currency-pass)** — ⏳ **open**. PR #57 자동화의 trigger 가드는 `auto_bypass_commit_pushed==True` (marker 푸시됨)을 요구. 그러나 새 서브타입 발견: rate-limit 감지 → manual `@coderabbitai review` post → CodeRabbit이 `Action performed: review triggered`만 ack하고 **decline도 review도 안 함** → B3-1d hybrid의 stage-2(marker push)는 decline marker를 보지 못하면 트리거 안 됨 → 600s + 1800s 누적 deadline 소진 후 timeout. 결과: `manual_attempted=True && commit_pushed=False`로 review-wait가 `failed`. PR #57 가드는 `commit_pushed=True`만 인정하므로 자동 회복 발동 안 함 → 운영자 수동 close+reopen 필요. PR #60 round 1에서 production 첫 케이스. Fix 후보: (a) recovery 가드를 `manual_attempted OR commit_pushed`로 확장 — 가장 간결, marker 부재 시에도 close+reopen이 합리적 (CodeRabbit 캐시 reset 효과는 marker와 무관), (b) B3-1d hybrid를 변경하여 manual ack 후 폴 N회 응답 없으면 marker 강제 push (운영 노이즈 ↑), (c) recovery 가드 그대로 두고 운영자 매뉴얼 절차로 유지 (현 상태). 1차 결정: (a) — 자동화 가드 한 줄 변경 + 1 추가 테스트, 운영 부담 ↓. 빈도 추가 누적 후 진행.
