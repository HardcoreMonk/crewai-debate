...
- **#13 Empty-commit fresh-SHA doesn't always elicit CodeRabbit re-review (신규, PR #45 dogfood-impl-timeout-override 검증 중 발견)** — ⏳ **open**. B3-1d hybrid auto-bypass는 의도대로 정확히 작동(rate-limit 감지 → manual `@coderabbitai review` post → CodeRabbit decline 응답 감지 → empty commit `0fe623f0` push, log timeline 검증됨). 그러나 push된 fresh SHA에 대해 CodeRabbit이 30+분 동안 review 미생성, 추가 rate-limit comment도 안 옴. 가능 원인: (a) CodeRabbit hourly bucket 완전 소진 시 silent ignore (추가 rate-limit comment도 안 보냄), (b) empty commit을 "no diff" filter로 자동 skip, (c) "already reviewed commits" 처리가 후속 push까지 일정 시간 적용. 실증 안 됨 — CodeRabbit 내부 동작 가시성 부족. Impact: B3-1d가 부분 효과 — Stage 1(manual)이 cheap하게 무동작 검증되고 Stage 2(empty commit)가 fire하지만 fresh review를 항상 받는 건 아님. Fix 후보: (a) empty commit 대신 trivial whitespace-only file edit (no-op이 아닌 진짜 commit)으로 push, (b) auto-bypass 후 일정 polling 안에 review 안 오면 또 empty commit 시도 (loop with cap), (c) 최종 fallback으로 PR 닫고 재오픈 (RUNBOOK 항목 (3) 자동화). 디자인 결정 필요 — (a)가 가장 단순하지만 commit이 실제 변경을 가져 review-apply 사이클에 영향 가능. 다음 dogfood에서 빈도 확인 후 우선순위 결정.
