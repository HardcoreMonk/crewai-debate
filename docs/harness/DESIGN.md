...
- **#12 Nitpick suggestion embedded in review body, not inline (신규, PR #30 dogfood gen-5 검증 중 발견)** — ⏳ **open**. CodeRabbit이 nitpick-only 포맷에서 actionable_count=N>0이라고 헤더에 표시하지만, 해당 suggestion은 **inline comment endpoint(`pulls/<n>/comments`)에 등장하지 않고** review body 안의 `<details>` 블록(보통 `♻️ Proposed cleanup` summary + ```diff 펜스)으로 embedded된 케이스가 존재. §13.6 #11 fix는 review body 헤더로 phase 분류만 담당하므로 review-fetch가 0건 반환 → review-apply가 no-op → suggestion 자동 적용 안 됨. PR #30에서 정확히 이 케이스(strict-failure note 중복 제거 cleanup)가 unapplied로 머지됨. 비치명적: gate는 clean하고 merge는 정상 진행되며 nitpick은 정의상 blocking이 아님. 다만 운영자 사후 검토에서 unapplied suggestion이 보이면 혼동 가능. Fix 후보 (다음 사이클 후속): (a) `coderabbit.py`에 review body 안의 `<details><summary>` 블록을 추출하는 별도 parser 추가 → review-fetch가 inline + body-embedded 합집합 반환, (b) review-fetch가 actionable_count > inline_count일 때 자동으로 (a) 폴백, (c) review-reply가 명시 경고("actionable count from header N, inline 0 — body-embedded suggestions may not have been auto-applied"). 디자인 결정 필요: (a)+(b)는 자동화 강화이고 (c)는 운영자 awareness만 보강 — 어느 것이 적정한지는 빈도에 의존.
